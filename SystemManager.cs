        ...
        public SystemManager(Configuration.ConfigData.Configuration config, CrestronControlSystem cs)
        {
            ...
            if (config.Rooms != null)
            {
                foreach (var room in config.Rooms) 
                {
                    try
                    {
                        this.rm = new Room.RoomConfig((ControlSystem)cs, room.Number, room.Name, room.SubSystemScenario, room.AudioSrcScenario, room.AudioSrcSharingScenario, room.SleepScenario, room.NAXBoxNumber, room.AudioID, room.VideoAudioID, room.VideoNAXBoxNumber, room.LightsID, room.ShadesID, room.ClimateID, room.MiscID, room.OpenSubsysNumOnRmSelect, room.ImageURL);
                        {
            ...
            if (config.Subsystems != null) {
                foreach (var subsystem in config.Subsystems)
                {
                    try
                    {
                        ...
                        this.subSys = new Subsystem.SubsystemConfig(subsystem.Number, subsystem.Name, subsystem.DisplayName, subsystem.IconSerial, subsystem.IconHTML, subsystem.AnalogModeNumber, subsystem.FlipsToPageNumber, subsystem.EquipID, subsystem.EISCIPaddress, hexId, subsystem.GuiScenarioNumber, subsystem.QuickActionsEnabled, subsystem.EISCExtraIPIDsHex, subsystem.ShadesEISCIPIDHex, subsystem.ShadesEISCExtraIPIDsHex);
